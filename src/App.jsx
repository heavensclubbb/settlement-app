...
function claimDestination(settlement) {
  return (
    settlement?.raw?.claim_form_url ||
    settlement?.raw?.source_url ||
    settlement?.id ||
    ""
  );
}
...
function SettlementDetail({ settlement, onBack }) {
  const s = settlement;
  const r = s.raw;
  const filingUrl = claimDestination(s);
  const classPeriod =
  ...
  return (
    <div style={{ background: paper, minHeight: "100%" }}>
      ...
      <div style={{ padding: "22px 20px" }}>
        ...
        >
          {r.claim_form_url ? "File claim on official site" : "View filing instructions"}
        </a>
      </div>
    </div>
  );
