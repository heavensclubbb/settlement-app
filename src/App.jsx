...
function useUserSettlements(userId) {
  ...
  const refresh = () => {
    ...
    supabase
      ...
      .select("settlement_url, status")
      .then(({ data }) => {
        const map = {};
        (data || []).forEach((row) => (map[row.settlement_url] = row.status));
        setStatusMap(map);
      });
  };
  ...
  const markStatus = async (settlementUrl, status) => {
    ...
    if (status === "seen" && statusMap[settlementUrl] === "filed") return;
    await supabase
      .from("user_settlements")
      .upsert(
        { user_id: userId, settlement_url: settlementUrl, status, updated_at: new Date().toISOString() },
        { onConflict: "user_id,settlement_url" }
      );
    setStatusMap((prev) => ({ ...prev, [settlementUrl]: status }));
  };
...
function ProfileScreen({ auth }) {
  const { session, signInWithEmail, signOut } = auth;
    ...
    <div style={{ background: paper, minHeight: "100%" }}>
      ...
      <div style={{ padding: "20px" }}>
        ...
        >
          ...
          {session === null && !sent && (
            <>
              ...
              <button
                ...
                style={{
                  width: "100%",
                  background: !email || sending ? "#C9CBCF" : ink,
                  color: paper,
                  border: "none",
                  borderRadius: 10,
                  padding: "12px 0",
          ...
          {session === null && sent && (
            <div style={{ fontFamily: sans, fontSize: 13, color: ink, lineHeight: 1.5 }}>
